...
--Q1
SELECT F1.IDCustomer, C1.Customer_Name,STATE,YEAR(DATE) AS DURATION_YEARS
FROM FACT_TRANSACTIONS F1
LEFT JOIN DIM_LOCATION  L1 ON F1.IDLocation=L1.IDLocation
LEFT JOIN DIM_CUSTOMER C1 ON F1.IDCustomer=C1.IDCustomer
WHERE YEAR(DATE) BETWEEN '2005' AND GETDATE()


--Q2
SELECT  TOP 1 L1.COUNTRY, L1.STATE, COUNT(L1.STATE) AS MAX_SAMSUNG_CELLPHONE
FROM DIM_LOCATION L1
LEFT JOIN FACT_TRANSACTIONS F1 ON F1.IDLocation=L1.IDLocation
LEFT JOIN DIM_MODEL M1 ON M1.IDModel = F1.IDModel
LEFT JOIN DIM_MANUFACTURER M2 ON M2.IDManufacturer=M1.IDManufacturer
WHERE M2.Manufacturer_Name ='SAMSUNG' AND L1.COUNTRY='US'
GROUP BY 
L1.COUNTRY,L1.STATE
ORDER BY 
MAX_SAMSUNG_CELLPHONE DESC
 

 --Q3
SELECT Manufacturer_Name,M1.Model_Name,COUNT(F1.IDModel) AS NUM_TRANSACTIONS,L1.State,L1.ZipCode
FROM DIM_MANUFACTURER MNFCTR
LEFT JOIN DIM_MODEL M1 ON MNFCTR.IDManufacturer=M1.IDManufacturer
LEFT JOIN FACT_TRANSACTIONS F1 ON M1.IDModel = F1.IDModel
LEFT JOIN DIM_LOCATION  L1 ON L1.IDLocation=F1.IDLocation
GROUP BY
 M1.Model_Name,Manufacturer_Name,L1.State,L1.ZipCode
ORDER BY 
L1.State,L1.ZipCode ASC
...
LEFT JOIN DIM_MODEL M1 ON M1.IDModel = F1.IDModel
LEFT JOIN DIM_MANUFACTURER MNFCTR ON MNFCTR.IDManufacturer=M1.IDManufacturer
ORDER BY
Unit_price ASC
...
HAVING
Manufacturer_Name IN (SELECT TOP 5 Manufacturer_Name FROM 
FACT_TRANSACTIONS F1
LEFT JOIN DIM_MODEL M1 ON M1.IDModel = F1.IDModel
LEFT JOIN DIM_MANUFACTURER MNFCTR ON MNFCTR.IDManufacturer=M1.IDManufacturer 
GROUP BY 
...
--Q6
SELECT
F1.IDCustomer, C1.Customer_Name,AVG(TOTALPRICE) as AVG_PRICE
FROM FACT_TRANSACTIONS F1
LEFT JOIN DIM_CUSTOMER C1 ON F1.IDCustomer=C1.IDCustomer
WHERE YEAR(DATE)='2009'
GROUP BY
F1.IDCustomer,C1.Customer_Name
HAVING
AVG(TOTALPRICE)>500
ORDER BY 
Customer_Name ASC 
...
YEAR(DATE) AS YEAR_MODELITEM,
SUM(QUANTITY) AS QTY
FROM DIM_MODEL M1
INNER JOIN FACT_TRANSACTIONS F1 ON M1.IDModel = F1.IDModel
WHERE YEAR(DATE) ='2008'-- AND YEAR(DATE) ='2009' AND YEAR(DATE) ='2010'
GROUP BY 
Model_Name,YEAR(DATE)
ORDER BY
QTY DESC
...
Model_Name,YEAR(DATE)
ORDER BY
QTY DESC) T2 ON T1.Model_Name=T2.Model_Name
INNER JOIN
(SELECT TOP 5
Model_Name,
YEAR(DATE) AS YEAR_MODELITEM,
...
Model_Name,YEAR(DATE)
ORDER BY
QTY DESC) T3 ON T2.Model_Name=T3.Model_Name
...
MANUFACTURER_NAME
ORDER BY 
MODEL_SALES DESC)AS T1
ORDER BY MODEL_SALES ASC
) T1 FULL JOIN (
SELECT TOP 1 * FROM (
SELECT TOP 2
MANUFACTURER_NAME,
...
MANUFACTURER_NAME
ORDER BY 
MODEL_SALES DESC) AS T2
ORDER BY MODEL_SALES ASC
) T2 ON T1.Manufacturer_Name=T2.Manufacturer_Name
...
--Q10
SELECT 
Customer_Name,YEAR(DATE) AS YEAR_ITEM,AVG(TOTALPRICE) AS AVG_PRICE,
--ROW_NUMBER() OVER (PARTITION BY Customer_Name ORDER BY YEAR(DATE) ASC) AS ROW_NUM,
LEAD(AVG(TOTALPRICE),1)OVER (PARTITION BY Customer_Name ORDER BY YEAR(DATE) ASC) AS LEAD_VAL,
(((AVG(TOTALPRICE)- ( LEAD(AVG(TOTALPRICE),1)OVER (PARTITION BY Customer_Name ORDER BY YEAR(DATE) ASC)))*100)/AVG(TOTALPRICE)) AS PERCENT_DIFF,
AVG(QUANTITY) AS AVG_QTY
FROM DIM_CUSTOMER C1
LEFT JOIN FACT_TRANSACTIONS F1 ON F1.IDCustomer=C1.IDCustomer
GROUP BY 
Customer_Name,YEAR(DATE)
HAVING 
Customer_Name IN 
(SELECT TOP 10 Customer_Name FROM DIM_CUSTOMER C1
LEFT JOIN FACT_TRANSACTIONS F1 ON F1.IDCustomer=C1.IDCustomer 
GROUP BY Customer_Name ORDER BY (AVG(TOTALPRICE))DESC)
ORDER BY
Customer_Name ASC,
YEAR_ITEM ASC
